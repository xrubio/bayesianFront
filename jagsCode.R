...
jagsHeteroSpeed<-function(x,error,distance,calCurves='intcal13',init=NA,raw=FALSE,plot=TRUE)
{
	...
	model{
		for (i in 1:nDate) {
			theta[i] ~ dnorm(mean[i],pow(alpha.g + beta.g *d[i],-2))
		        mean[i] <- alpha.m + beta.m * d[i]
			mu[i] <- interp.lin(theta[i], calBP[], C14BP[])
			sigmaCurve[i] <- interp.lin(theta[i], calBP[], C14err[])
			tau[i] <- 1/(pow(sigma[i],2)+pow(sigmaCurve[i],2))
			X[i] ~ dnorm(mu[i],tau[i])
	...
	jagsModel=jags.model(file=textConnection(modelString),data=dataList,inits=initsList,n.chains=1)
	update(jagsModel,n.iter=3000)
	codaSamples=coda.samples(jagsModel,variable.names=c("one.year","beta.m","alpha.m","beta.g","alpha.g"),n.iter=50000)
        return(codaSamples)
}
